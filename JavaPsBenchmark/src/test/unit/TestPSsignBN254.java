...
import pairingBN254.PairingBuilderBN254;
import pairingBN254.ZpElementBN254;
...
public class TestPSsignBN254 {
	...
	public void testCompletePSFlow() throws MSSetupException {
		...
		for(String attr:attrNames){
			attributes.put(attr,new ZpElementBN254(BIG.randomnum(p, rng)));
		}
		ZpElement epoch=new ZpElementBN254(BIG.randomnum(p, rng));
	...
	public void testDifferentAttributesVerify() throws MSSetupException {
		...
		for(String attr:attrNames){
			wrongAttributes.put(attr,new ZpElementBN254(BIG.randomnum(p, rng)));
		}
	...
	public void testWrongEpoch() throws MSSetupException {
		...
		MSsignature signature=psScheme.comb(serverVK,signShares);
		//Creating new attributes
		ZpElement wrongEpoch=new ZpElementBN254(BIG.randomnum(p, rng));
		//Creating message from falsified attributes
	...
	public void testSignExceptions() throws MSSetupException {
		...
		attributes2.put("test",new ZpElementBN254(BIG.randomnum(p, rng)));
		MSmessage wrongNattrMsg=new PSmessage(attributes2,epoch);
		Map<String, ZpElement> attributes3=new HashMap<>();
		attributes3.put("test1",new ZpElementBN254(BIG.randomnum(p, rng)));
		attributes3.put("test2",new ZpElementBN254(BIG.randomnum(p, rng)));
	...
	public void testCombExceptions() throws MSSetupException {
		...
		signSharesWrong1[nServers-1]=new PSsignature(new PairingBuilderBN254().getRandomZpElement(),((PSsignature)signShares[nServers-1]).getSigma1(),((PSsignature)signShares[nServers-1]).getSigma2());
		MSsignature[] signSharesWrong2=new MSsignature[nServers];
		for(int i=0;i<nServers-1;i++){
			signSharesWrong2[i]=signShares[i];
		}
		signSharesWrong2[nServers-1]=new PSsignature(((PSsignature)signShares[nServers-1]).getMPrim(),new PairingBuilderBN254().getGroup1Generator(),((PSsignature)signShares[nServers-1]).getSigma2());
	...
	public void testVerfFraudulentUnitySignature() throws MSSetupException {
		...
		//Combining shares in one signature
		MSsignature signature=psScheme.comb(serverVK,signShares);
		PairingBuilder pb=new PairingBuilderBN254();
