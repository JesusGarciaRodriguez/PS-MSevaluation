...
public class PairingBLS461 implements Pairing {
    ...
    @Override
    public Group3ElementBLS461 pair(Group1Element el1, Group2Element el2) {
        if(!(el1 instanceof Group1ElementBLS461))
            throw new IllegalArgumentException("el1 must be Group1ElementBLS461");
        if(!(el2 instanceof Group2ElementBLS461))
            throw new IllegalArgumentException("el2 must be Group2ElementBLS461");
        ECP e1= ((Group1ElementBLS461) el1).x;
        ECP2 e2= ((Group2ElementBLS461) el2).x;
    ...
    public Group3Element multiPair(Collection<Pair<Group1Element,Group2Element>> elements) {
        ...
        for(Pair<Group1Element,Group2Element> el:elements){
            if(!(el.getFirst() instanceof Group1ElementBLS461))
                throw new IllegalArgumentException("el1 must be Group1ElementBLS461");
            if(!(el.getSecond()   instanceof Group2ElementBLS461))
                throw new IllegalArgumentException("el2 must be Group2ElementBLS461");
            ECP e1= ((Group1ElementBLS461) el.getFirst()).x;
            ECP2 e2= ((Group2ElementBLS461) el.getSecond()).x;
