...
public class Hash0BN254 implements Hash0 {

    @Override
    public Pair<ZpElement, Group1Element> hash(Collection<ZpElement> m) {
        byte [] b=new byte[0];
        for(ZpElement mi: m) {
            if(!(mi instanceof ZpElementBN254))
                throw new IllegalArgumentException("Argument must be collection of ZpElementBN254");
            b=append(b, bigToBytes(((ZpElementBN254)mi).x));
        }
        ZpElement mPrim=new ZpElementBN254(hashModOrder(b));
        Group1Element h=new Group1ElementBN254(hashECP(b));
    ...
    private static byte[] bigToBytes(BIG big) {
        byte[] ret = new byte[PairingBN254.FIELD_BYTES];
        big.toBytes(ret);
        return ret;
    }
}
